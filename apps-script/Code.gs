...
function setupVisitorManagement() {
  ...
  return 'VISTA v1.2.0 setup complete. Review Config and Agreements, then deploy a new Web app version.';
}
...
function getVisitPhoto_(p) { validateRequired_(p,['visitId']); const rec=findVisitRow_(p.visitId).obj;if(!rec.PhotoFileId)return{ok:true,hasPhoto:false,dataUrl:''};try{const blob=DriveApp.getFileById(String(rec.PhotoFileId)).getBlob(),mime=blob.getContentType()||'image/jpeg';return{ok:true,hasPhoto:true,dataUrl:`data:${mime};base64,${Utilities.base64Encode(blob.getBytes())}`}}catch(err){throw new Error('Visitor photograph could not be loaded. Confirm Apps Script access to the configured photo folder.')} }
function savePhoto_(dataUrl,visitId){const m=String(dataUrl).match(/^data:(image\/[^;]+);base64,(.+)$/);if(!m)throw new Error('Invalid photograph data.');const cfg=config_(),folder=cfg.PHOTO_FOLDER_ID?DriveApp.getFolderById(cfg.PHOTO_FOLDER_ID):DriveApp.getRootFolder(),ext=m[1].includes('png')?'png':'jpg',blob=Utilities.newBlob(Utilities.base64Decode(m[2]),m[1],visitId+'.'+ext),file=folder.createFile(blob);return{id:file.getId(),url:`https://drive.google.com/uc?export=view&id=${file.getId()}`}}
