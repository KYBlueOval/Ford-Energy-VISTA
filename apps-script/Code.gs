const FE = {
  VERSION: '2.5.3-sponsor-self-service',
...
function setupVisitorManagement() {
  ...
  return 'VISTA 2.5.3 setup complete. Sponsor delegation, approval governance, administrator oversight, self-service scheduling, and existing workflows are ready.';
}
...
function notifySubmission_(r){
  const cfg=config_(),reviewUrl=cfg.SPONSOR_PORTAL_URL||'https://kyblueoval.github.io/Ford-Energy-VISTA/sponsor-portal/',securityUrl=cfg.SECURITY_CONSOLE_URL||'',sponsorCreated=String(r.SponsorSource||'')==='VISTA Sponsor Portal',result={sponsorSent:false,visitorSent:false,securitySent:false,securityConfigured:Boolean(cfg.NOTIFICATION_EMAIL),errors:[]};
...
function createSponsoredVisitAuthorized_(p,session){
  ...
  const delegations=activeSponsorDelegations_(new Date()),created=createdRecords.map(record=>({visit:publicSponsorVisit_(record,session,delegations),notifications:notifySubmission_(record)}));
