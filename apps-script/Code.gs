const FE = {
  VERSION: '2.6.0-sponsor-leadership-governance',
...
function listSponsorDashboard_(p,session){
  ...
  const leadership=activeSponsorLeadership_().filter(r=>String(r.LeaderUserID||'')===String(session.UserID||'')&&truthyActive_(r.ReportAccess));
  return{ok:true,generatedAt:dateTime_(now),oversight:sponsorOversightRole_(session),leadershipOversight:leadership.length>0||sponsorOversightRole_(session),sponsor:{fullName:String(session.FullName||session.Username||''),email:String(session.Email||''),department:String(session.Department||''),role:String(session.Role||'')},summary:{pending:pending,approvedUpcoming:approved,onsite:onsite,completed:completed,total:visits.length,delegatedToMe:incoming,delegatedByMe:outgoing,assignedSponsors:leadership.length},visits:visits};
}
