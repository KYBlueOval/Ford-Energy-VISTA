const FE = {
  VERSION: '2.5.2-sponsor-governance',
...
function setupVisitorManagement() {
  ...
  return 'VISTA 2.5.2 setup complete. Sponsor delegation, review reminders, escalation governance, and existing workflows are ready.';
}
...
function listSponsorDelegations_(p,session){const now=new Date(),rows=readObjects_(FE.SHEETS.SPONSOR_DELEGATIONS).filter(r=>String(r.SponsorUserID||'')===String(session.UserID||'')||String(r.DelegateUserID||'')===String(session.UserID||'')).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0)).slice(0,250).map(r=>publicSponsorDelegation_(r,now));return{ok:true,generatedAt:dateTime_(now),delegations:rows,summary:{activeOutgoing:rows.filter(r=>r.sponsorUserId===String(session.UserID||'')&&r.status==='Active').length,activeIncoming:rows.filter(r=>r.delegateUserId===String(session.UserID||'')&&r.status==='Active').length,scheduled:rows.filter(r=>r.status==='Scheduled').length}};}
