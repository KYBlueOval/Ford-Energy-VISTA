const FE = {
  ...
  SPONSOR_HIERARCHY_HEADERS: ['HierarchyID','CreatedAt','SponsorUserID','SponsorEmployeeID','SponsorUsername','SponsorName','SponsorEmail','LeaderUserID','LeaderEmployeeID','LeaderUsername','LeaderName','LeaderEmail','LeaderLevel','Status','NotificationsEnabled','ReportAccess','Notes','CreatedByUserID','CreatedBy','EndedAt','EndedByUserID','EndedBy','LastUpdatedAt'],
...
function leaderSponsorIdentities_(session){const userId=String(session&&session.UserID||'');return activeSponsorLeadership_().filter(r=>String(r.LeaderUserID||'')===userId&&truthyActive_(r.ReportAccess)).map(r=>({Email:r.SponsorEmail,EmployeeID:r.SponsorEmployeeID,Username:r.SponsorUsername}));}
...
function sponsorVisitAccess_(visit,session,delegations){
  ...
  if(leaderSponsorIdentities_(session).some(identity=>sponsorIdentityMatchesVisit_(visit,identity)))return{allowed:true,type:'Leadership Oversight',delegatedFrom:String(visit.SponsorName||visit.SponsorEmail||'Assigned sponsor')};
...
function listSponsorDashboard_(p,session){
  ...
  const now=new Date(),delegations=activeSponsorDelegations_(now),badgeRequests=latestPermanentBadgeRequestMap_(),rows=sponsorVisits_(p,session),visits=rows.map(r=>{const visit=publicSponsorVisit_(r,session,delegations),request=badgeRequests[String(r.VisitID||'')];visit.permanentBadgeRequest=request?publicPermanentBadgeRequest_(request):null;return visit;}),today=dateKey_(now),pending=visits.filter(v=>['Submitted','Pending Review','Pending Sponsor Review'].includes(v.status)).length,approved=visits.filter(v=>v.status==='Approved'&&v.startDate>=today).length,onsite=visits.filter(v=>v.status==='Checked In').length,completed=visits.filter(v=>v.status==='Checked Out').length,incoming=delegations.filter(r=>String(r.DelegateUserID||'')===String(session.UserID||'')).length,outgoing=delegations.filter(r=>String(r.SponsorUserID||'')===String(session.UserID||'')).length;
  const leadership=activeSponsorLeadership_().filter(r=>String(r.LeaderUserID||'')===String(session.UserID||'')&&truthyActive_(r.ReportAccess)),enterpriseOversight=sponsorOversightRole_(session)||plantLeaderRole_(session);
...
function publicSponsorLeadership_(r){return{hierarchyId:String(r.HierarchyID||''),createdAt:dateText_(r.CreatedAt),sponsorUserId:String(r.SponsorUserID||''),sponsorName:String(r.SponsorName||''),sponsorEmail:String(r.SponsorEmail||''),leaderUserId:String(r.LeaderUserID||''),leaderName:String(r.LeaderName||''),leaderEmail:String(r.LeaderEmail||''),leaderLevel:String(r.LeaderLevel||''),status:String(r.Status||''),notificationsEnabled:truthyActive_(r.NotificationsEnabled),reportAccess:truthyActive_(r.ReportAccess),notes:String(r.Notes||'')};}
function listSponsorLeadershipAuthorized_(p,session){const admin=sponsorOversightRole_(session),userId=String(session.UserID||''),rows=activeSponsorLeadership_().filter(r=>admin||String(r.SponsorUserID||'')===userId||String(r.LeaderUserID||'')===userId).map(publicSponsorLeadership_);return{ok:true,oversight:admin,assignments:rows,canViewReports:admin||rows.some(r=>r.leaderUserId===userId&&r.reportAccess)};}
function saveSponsorLeadershipAuthorized_(p,session){validateRequired_(p,['sponsorUserId','leaderUserId','leaderLevel']);const sponsor=activeSponsorUser_(p.sponsorUserId),leader=activeSponsorUser_(p.leaderUserId);if(String(sponsor.UserID)===String(leader.UserID))throw new Error('A Sponsor cannot be assigned as their own Plant Leader.');const duplicate=activeSponsorLeadership_().find(r=>String(r.SponsorUserID)===String(sponsor.UserID)&&String(r.LeaderUserID)===String(leader.UserID));if(duplicate)throw new Error('This Sponsor and Plant Leader assignment is already active.');const now=new Date(),record={HierarchyID:'SLH-'+Utilities.getUuid().slice(0,12).toUpperCase(),CreatedAt:now,SponsorUserID:sponsor.UserID,SponsorEmployeeID:sponsor.EmployeeID,SponsorUsername:sponsor.Username,SponsorName:sponsor.FullName,SponsorEmail:sponsor.Email,LeaderUserID:leader.UserID,LeaderEmployeeID:leader.EmployeeID,LeaderUsername:leader.Username,LeaderName:leader.FullName,LeaderEmail:leader.Email,LeaderLevel:String(p.leaderLevel).trim().slice(0,40),Status:'Active',NotificationsEnabled:truthy_(p.notificationsEnabled===undefined?'Yes':p.notificationsEnabled)?'Yes':'No',ReportAccess:truthy_(p.reportAccess===undefined?'Yes':p.reportAccess)?'Yes':'No',Notes:String(p.notes||'').trim().slice(0,1000),CreatedByUserID:session.UserID,CreatedBy:session.FullName||session.Username,LastUpdatedAt:now};appendObjectRow_(SpreadsheetApp.getActive().getSheetByName(FE.SHEETS.SPONSOR_HIERARCHY),record,FE.SPONSOR_HIERARCHY_HEADERS);notifySponsorLeadershipAssignment_(record);audit_(session,'SPONSOR_LEADERSHIP_ASSIGNED','','','Success',record.SponsorName+' → '+record.LeaderName+' · '+record.LeaderLevel);return{ok:true,assignment:publicSponsorLeadership_(record),message:'Plant leadership oversight assignment created.'};}
...
function notifySponsorLeadershipAssignment_(record){if(!truthyActive_(config_().SPONSOR_LEADERSHIP_EMAILS_ENABLED)||!validEmailAddress_(record.LeaderEmail))return{sent:false,status:'Not requested'};try{const portal=config_().SPONSOR_PORTAL_URL||'https://kyblueoval.github.io/Ford-Energy-VISTA/sponsor-portal/',body=`<p>Hello ${html_(record.LeaderName)},</p><p>You have been assigned ${html_(record.LeaderLevel)} oversight for VISTA Sponsor <strong>${html_(record.SponsorName)}</strong>.</p><p>You can review Sponsor performance and the current/next-week visitor outlook in the Sponsor Portal. Operational actions remain attributed to the person who performs them.</p>${emailButton_(portal,'Open Leadership Oversight')}`;MailApp.sendEmail({to:record.LeaderEmail,subject:'VISTA Sponsor leadership oversight assigned',body:`You have been assigned oversight for ${record.SponsorName}. Open ${portal}`,htmlBody:brandedEmail_({title:'Sponsor Oversight Assigned',kicker:'FORD ENERGY · VISTA LEADERSHIP',body:body})});return{sent:true,status:'Sent'};}catch(e){return{sent:false,status:'Failed',error:String(e&&e.message||e)};}}
