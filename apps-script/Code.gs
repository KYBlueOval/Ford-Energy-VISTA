const FE = {
  VERSION: '2.3.7-mobile-operations',
  SHEETS: { VISITS:'VisitRequests', ACTIVITY:'VisitActivity', BADGES:'BadgeInventory', CONFIG:'Config', AGREEMENTS:'Agreements', ACKS:'AgreementAcknowledgements', SPONSORS:'Sponsors', USERS:'Users', FRONTDESK:'FrontDesk', SECURITY:'Security', SESSIONS:'AuthSessions', AUDIT:'AuditLog', HANDOFFS:'ShiftHandoffs', NOTIFICATIONS:'Notifications', INCIDENTS:'Incidents' },
  ...
  NOTIFICATION_HEADERS: ['NotificationID','CreatedAt','Type','Severity','Title','Message','RelatedVisitID','RelatedBadgeUID','RelatedHandoffID','Status','AcknowledgedAt','AcknowledgedByUserID','AcknowledgedBy','LastUpdatedAt','DeduplicationKey'],
...
function setupVisitorManagement() {
  ...
  seedAgreements_();
  seedInitialAdmin_();
  [FE.SHEETS.VISITS,FE.SHEETS.ACTIVITY,FE.SHEETS.BADGES,FE.SHEETS.AGREEMENTS,FE.SHEETS.ACKS,FE.SHEETS.SPONSORS,FE.SHEETS.USERS,FE.SHEETS.FRONTDESK,FE.SHEETS.SECURITY,FE.SHEETS.SESSIONS,FE.SHEETS.AUDIT,FE.SHEETS.HANDOFFS,FE.SHEETS.NOTIFICATIONS,FE.SHEETS.INCIDENTS].forEach(n=>ss.getSheetByName(n).setFrozenRows(1));
  return 'VISTA 2.3.7 setup complete. Identity, badge, audit, handoff, alert, incident, analytics, and reporting sources are ready.';
}
...
function publicOperationalNotification_(r){return{notificationId:String(r.NotificationID||''),createdAt:dateText_(r.CreatedAt),type:String(r.Type||''),severity:String(r.Severity||'Medium'),title:String(r.Title||''),message:String(r.Message||''),visitId:String(r.RelatedVisitID||''),badgeUid:String(r.RelatedBadgeUID||''),handoffId:String(r.RelatedHandoffID||''),status:String(r.Status||'Open'),acknowledgedAt:dateText_(r.AcknowledgedAt),acknowledgedBy:String(r.AcknowledgedBy||'')};}
function listOperationalNotifications_(p,session){synchronizeOperationalNotifications_(session);const includeCleared=Boolean(p.includeCleared),limit=Math.min(100,Math.max(1,Number(p.limit||25))),rows=readObjects_(FE.SHEETS.NOTIFICATIONS).filter(r=>includeCleared||String(r.Status||'Open')!=='Cleared').slice(-limit).reverse().map(publicOperationalNotification_);return{ok:true,generatedAt:dateTime_(new Date()),notifications:rows,summary:{active:rows.filter(r=>r.status!=='Cleared').length,critical:rows.filter(r=>r.status!=='Cleared'&&r.severity==='Critical').length,unacknowledged:rows.filter(r=>r.status==='Open').length}};}
...
function acknowledgeOperationalNotification_(p,session){validateRequired_(p,['notificationId']);const found=findNotificationRow_(p.notificationId);if(String(found.obj.Status)==='Cleared')throw new Error('This alert is already cleared.');const now=new Date(),by=String(session.FullName||session.Username||'');updateObjectRow_(FE.SHEETS.NOTIFICATIONS,found.row,{Status:'Acknowledged',AcknowledgedAt:now,AcknowledgedByUserID:String(session.UserID||''),AcknowledgedBy:by,LastUpdatedAt:now});audit_(session,'NOTIFICATION_ACKNOWLEDGED',String(found.obj.RelatedVisitID||''),'','Success',String(found.obj.NotificationID||'')+' · '+String(found.obj.Title||''));return{ok:true};}
