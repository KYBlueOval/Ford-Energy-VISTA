const FE = {
  VERSION: '2.5.6-permanent-badge-authorization',
...
function setupVisitorManagement() {
  ...
  seedInitialAdmin_();
  [FE.SHEETS.VISITS,FE.SHEETS.ACTIVITY,FE.SHEETS.BADGES,FE.SHEETS.AGREEMENTS,FE.SHEETS.ACKS,FE.SHEETS.SPONSORS,FE.SHEETS.USERS,FE.SHEETS.FRONTDESK,FE.SHEETS.SECURITY,FE.SHEETS.SESSIONS,FE.SHEETS.AUDIT,FE.SHEETS.HANDOFFS,FE.SHEETS.NOTIFICATIONS,FE.SHEETS.NOTIFICATION_ACKS,FE.SHEETS.INCIDENTS,FE.SHEETS.EV_REQUESTS,FE.SHEETS.EV_ACTIVITY,FE.SHEETS.EV_POLICIES,FE.SHEETS.SPONSOR_DELEGATIONS,FE.SHEETS.SPONSOR_REMINDERS,FE.SHEETS.PERMANENT_BADGE_REQUESTS].forEach(n=>ss.getSheetByName(n).setFrozenRows(1));
  return 'VISTA 2.5.6 setup complete. Permanent badge authorization, visitor onboarding delivery, Global Security notification, and Administration tracking are ready.';
}
...
function createPermanentBadgeRequestAuthorized_(p,session){
  validateRequired_(p,['visitId','badgeType','accessRequired','visitPattern','businessJustification']);const row=requireSponsorOwnedVisit_(p.visitId,session),visit=row.obj,status=String(visit.Status||''),allowedTypes=['Ford Energy (Blue)','Ford Energy Contractor (Green)','Ford Energy OEM (Orange)'];
...
function listPermanentBadgeRequestsAuthorized_(p,session){const query=String(p.query||'').trim().toLowerCase(),status=String(p.status||'').trim();let rows=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS);rows=rows.filter(r=>!status||String(r.Status||'')===status).filter(r=>!query||[r.RequestID,r.VisitorName,r.VisitorEmail,r.Company,r.BadgeType,r.AccessRequired,r.SponsorName,r.ConfirmationNumber,r.IssuedBadgeNumber,r.IssuedBadgeUID].join(' ').toLowerCase().includes(query)).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0));const all=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS),count=s=>all.filter(r=>String(r.Status||'')===s).length;return{ok:true,generatedAt:dateTime_(new Date()),summary:{total:all.length,pending:count('Authorized - Pending Security Processing'),production:count('In Production'),issued:count('Issued'),closed:count('Closed')},requests:rows.slice(0,500).map(publicPermanentBadgeRequest_)};}
function updatePermanentBadgeRequestStatusAuthorized_(p,session){validateRequired_(p,['requestId','status']);const allowed=['Authorized - Pending Security Processing','In Production','Issued','Closed','Cancelled'],status=String(p.status);if(!allowed.includes(status))throw new Error('Select a valid permanent badge processing status.');if(status==='Issued'&&!String(p.issuedBadgeNumber||'').trim())throw new Error('Enter the issued badge number before marking this request Issued.');const found=findPermanentBadgeRequest_(p.requestId),now=new Date(),updates={Status:status,ProcessingNotes:String(p.processingNotes||'').trim().slice(0,2000),IssuedBadgeNumber:String(p.issuedBadgeNumber||found.obj.IssuedBadgeNumber||'').trim(),IssuedBadgeUID:String(p.issuedBadgeUid||found.obj.IssuedBadgeUID||'').trim(),LastUpdatedAt:now};if(['Issued','Closed','Cancelled'].includes(status)){updates.CompletedAt=now;updates.CompletedByUserID=String(session.UserID||'');updates.CompletedBy=String(session.FullName||session.Username||'');}updateObjectRow_(FE.SHEETS.PERMANENT_BADGE_REQUESTS,found.row,updates);const details=[found.obj.RequestID,status,updates.IssuedBadgeNumber?'Badge '+updates.IssuedBadgeNumber:'',updates.ProcessingNotes].filter(Boolean).join(' · ');logActivity_(found.obj.VisitID,'PERMANENT_BADGE_STATUS_UPDATED',session.FullName||session.Username,'',details);audit_(session,'ADMIN_PERMANENT_BADGE_STATUS',found.obj.VisitID,found.obj.ConfirmationNumber,'Success',details);return{ok:true,message:'Permanent badge request status updated.',request:publicPermanentBadgeRequest_(Object.assign({},found.obj,updates))};}
