const FE = {
  VERSION: '2.6.4-sponsor-hierarchy-responsibility-ui',
  ...
  SPONSOR_HIERARCHY_HEADERS: ['HierarchyID','CreatedAt','SponsorUserID','SponsorEmployeeID','SponsorUsername','SponsorName','SponsorEmail','OCMSponsorUserID','OCMSponsorEmployeeID','OCMSponsorUsername','OCMSponsorName','OCMSponsorEmail','OCMSponsorLevel','LeaderUserID','LeaderEmployeeID','LeaderUsername','LeaderName','LeaderEmail','LeaderLevel','Status','NotificationsEnabled','ReportAccess','Notes','CreatedByUserID','CreatedBy','EndedAt','EndedByUserID','EndedBy','LastUpdatedAt'],
  VISIT_RESPONSIBILITY_HEADERS: ['AssignmentID','VisitID','ConfirmationNumber','CreatedAt','PrimarySponsorUserID','PrimarySponsorName','PrimarySponsorEmail','OperationalSponsorUserID','OperationalSponsorEmployeeID','OperationalSponsorName','OperationalSponsorEmail','Status','Reason','AssignedByUserID','AssignedBy','EndedAt','EndedByUserID','EndedBy','LastUpdatedAt'],
  PERMANENT_BADGE_REQUEST_HEADERS: ['RequestID','CreatedAt','Status','VisitID','ConfirmationNumber','VisitorName','VisitorEmail','Company','VisitorPhotoFileId','VisitorPhotoFileName','VisitorPhotoUrl','BadgeType','AccessRequired','VisitPattern','BusinessJustification','SponsorUserID','SponsorName','SponsorEmail','SponsorRole','SponsorAccessType','OnboardingIntakeUrl','VisitorNotificationSent','VisitorNotificationStatus','VisitorNotificationAt','VisitorNotificationError','SecurityNotificationSent','SecurityNotificationStatus','SecurityNotificationRecipients','SecurityNotificationAt','SecurityNotificationError','ProcessingNotes','IssuedBadgeNumber','IssuedBadgeUID','CompletedAt','CompletedByUserID','CompletedBy','LastUpdatedAt'],
...
function doGet(e) {
  ...
  try {
    ...
    if (action === 'listSponsors') {
      ...
      result = {ok:true,advanceNoticeRequiredDays:advanceNoticeRequiredDays_()};
    } else {
...
function doPost(e) {
  ...
  try {
    ...
    if(action==='createVisit') result=createVisit_(req.payload||{});
      ...
      else if(action==='listOperationalSponsorDirectory') { requirePermission_(session,'sponsorPortal'); result=listOperationalSponsorDirectory_(req.payload||{},session); }
      ...
      else if(action==='listSponsorLeadership') { requirePermission_(session,'sponsorPortal'); result=listSponsorLeadershipAuthorized_(req.payload||{},session); }
      else if(action==='getLeadershipReports') { requirePermission_(session,'sponsorReports'); result=getLeadershipReportsAuthorized_(req.payload||{},session); }
      else if(action==='assignOperationalSponsor') { requirePermission_(session,'sponsorPortal'); result=assignOperationalSponsorAuthorized_(req.payload||{},session); }
      else if(action==='clearOperationalSponsor') { requirePermission_(session,'sponsorPortal'); result=clearOperationalSponsorAuthorized_(req.payload||{},session); }
      else if(action==='createPermanentBadgeRequest') { requirePermission_(session,'sponsorPortal'); result=createPermanentBadgeRequestAuthorized_(req.payload||{},session); }
      ...
      else if(action==='saveSponsorLeadership') { requirePermission_(session,'manageConfig'); result=saveSponsorLeadershipAuthorized_(req.payload||{},session); }
      else if(action==='endSponsorLeadership') { requirePermission_(session,'manageConfig'); result=endSponsorLeadershipAuthorized_(req.payload||{},session); }
      else if(action==='sendSponsorLeadershipReports') { requirePermission_(session,'manageConfig'); result=sendSponsorLeadershipReportsAuthorized_(req.payload||{},session); }
      ...
      else if(action==='listPermanentBadgeRequests') { requirePermission_(session,'manageConfig'); result=listPermanentBadgeRequestsAuthorized_(req.payload||{},session); }
      else if(action==='updatePermanentBadgeRequestStatus') { requirePermission_(session,'manageConfig'); result=updatePermanentBadgeRequestStatusAuthorized_(req.payload||{},session); }
...
function setupVisitorManagement() {
  ...
  const visitSheet=ensureSheet_(ss, FE.SHEETS.VISITS, FE.VISIT_HEADERS);
  formatVisitGovernanceColumns_(visitSheet);
  ...
  ensureSheet_(ss, FE.SHEETS.SPONSOR_HIERARCHY, FE.SPONSOR_HIERARCHY_HEADERS);
  ensureSheet_(ss, FE.SHEETS.VISIT_RESPONSIBILITY, FE.VISIT_RESPONSIBILITY_HEADERS);
  ensureSheet_(ss, FE.SHEETS.PERMANENT_BADGE_REQUESTS, FE.PERMANENT_BADGE_REQUEST_HEADERS);
  ...
  const defaults = {
    ...
    SPONSOR_LEADERSHIP_EMAILS_ENABLED:'YES',
    SPONSOR_LEADERSHIP_REPORTS_ENABLED:'YES',
    SPONSOR_LEADERSHIP_REPORT_DAY:'MONDAY',
    SPONSOR_LEADERSHIP_REPORT_HOUR:'6',
    ...
    PERMANENT_BADGE_VISITOR_EMAIL_ENABLED:'YES',
    PERMANENT_BADGE_SECURITY_EMAIL_ENABLED:'NO',
    PERMANENT_BADGE_SECURITY_EMAILS:'',
  ...
  return 'VISTA 2.6.4 setup complete. SponsorLeadership now supports Plant Leadership, OCM Sponsor, primary Sponsor, and visitor accountability.';
}
...
function seedEVChargingPolicy_(){
  ...
  const sitePolicy=[
    ...
    'Move the vehicle when charging is complete or when the 6-hour daily charging allotment has been reached so charging capacity remains available to other approved employees.',
...
function createVisit_(p) {
  ...
  Object.assign(record,advanceNoticeMetrics_(now,p.startDate,p.arrivalTime));
...
function sponsorPerformance_(visits){
  const groups={};
  visits.forEach(r=>{const email=String(r.SponsorEmail||'').trim().toLowerCase(),name=String(r.SponsorName||'Unassigned').trim()||'Unassigned',key=email||name.toLowerCase(),notice=advanceNoticeForVisit_(r),status=String(r.Status||'Submitted'),decided=['Approved','Checked In','Checked Out','Denied','Rejected','Cancelled','No Show'].includes(status),approved=['Approved','Checked In','Checked Out'].includes(status),response=sheetNumber_(r.ApprovalResponseMinutes,0);if(!groups[key])groups[key]={name:name,email:email,totalRequests:0,compliantRequests:0,lateRequests:0,pendingRequests:0,approvedRequests:0,deniedRequests:0,manualApprovals:0,responseTotal:0,responseSamples:0,leadTotal:0};const g=groups[key];g.totalRequests++;notice.compliant?g.compliantRequests++:g.lateRequests++;g.leadTotal+=Number(notice.actualDays||0);if(!decided)g.pendingRequests++;if(approved)g.approvedRequests++;if(['Denied','Rejected'].includes(status))g.deniedRequests++;if(approved&&String(r.ApprovalManualOverride||'')==='Yes')g.manualApprovals++;if(response>0){g.responseTotal+=response;g.responseSamples++;}});
  return Object.values(groups).map(g=>({name:g.name,email:g.email,totalRequests:g.totalRequests,compliantRequests:g.compliantRequests,lateRequests:g.lateRequests,complianceRate:g.totalRequests?Math.round(g.compliantRequests/g.totalRequests*100):0,pendingRequests:g.pendingRequests,approvedRequests:g.approvedRequests,deniedRequests:g.deniedRequests,manualApprovals:g.manualApprovals,averageResponseMinutes:g.responseSamples?Math.round(g.responseTotal/g.responseSamples):0,responseSamples:g.responseSamples,averageLeadDays:g.totalRequests?Math.round(g.leadTotal/g.totalRequests*10)/10:0})).sort((a,b)=>b.totalRequests-a.totalRequests||a.name.localeCompare(b.name));
}
function sponsorPerformancePopulation_(visits){const performance=sponsorPerformance_(visits),byKey=performance.reduce((map,row)=>(map[String(row.email||row.name).toLowerCase()]=row,map),{});readObjects_(FE.SHEETS.USERS).filter(user=>truthyActive_(user.Active)&&['sponsor','approver'].includes(String(user.Role||'').trim().toLowerCase())).forEach(user=>{const key=String(user.Email||user.FullName||user.Username).trim().toLowerCase();if(!byKey[key])byKey[key]={name:String(user.FullName||user.Username||'Sponsor'),email:String(user.Email||''),totalRequests:0,compliantRequests:0,lateRequests:0,complianceRate:0,pendingRequests:0,approvedRequests:0,deniedRequests:0,manualApprovals:0,averageResponseMinutes:0,responseSamples:0,averageLeadDays:0};});return Object.values(byKey).sort((a,b)=>b.totalRequests-a.totalRequests||a.name.localeCompare(b.name));}
function getOperationalAnalytics_(p,session){
  ...
  const topSponsors=Object.keys(sponsors).map(name=>({name:name,count:sponsors[name]})).sort((a,b)=>b.count-a.count||a.name.localeCompare(b.name)).slice(0,5),dailyVolume=Object.keys(daily).sort().map(date=>({date:date,count:daily[date]})),sponsorPerformance=sponsorPerformance_(visits),governance=sponsorPerformance.reduce((o,g)=>(o.compliantRequests+=g.compliantRequests,o.lateRequests+=g.lateRequests,o.manualApprovals+=g.manualApprovals,o.responseTotal+=g.averageResponseMinutes*g.responseSamples,o.responseSamples+=g.responseSamples,o),{compliantRequests:0,lateRequests:0,manualApprovals:0,responseTotal:0,responseSamples:0});
  governance.complianceRate=visits.length?Math.round(governance.compliantRequests/visits.length*100):0;governance.averageResponseMinutes=governance.responseSamples?Math.round(governance.responseTotal/governance.responseSamples):0;governance.advanceNoticeRequiredDays=advanceNoticeRequiredDays_();delete governance.responseTotal;delete governance.responseSamples;
  return{ok:true,generatedAt:dateTime_(now),range:{days:days,start:startKey,end:today},summary:{totalVisits:visits.length,approved:approved,denied:statuses.Denied,noShow:statuses['No Show'],checkedIn:statuses['Checked In'],checkedOut:statuses['Checked Out'],onsiteNow:allVisits.filter(r=>String(r.Status)==='Checked In').length,approvalRate:decided?Math.round(approved/decided*100):0,averageDurationMinutes:avgDuration,peakArrivalHour:peakHour===undefined?'—':String(peakHour).padStart(2,'0')+':00'},statuses:statuses,dailyVolume:dailyVolume,topSponsors:topSponsors,sponsorPerformance:sponsorPerformance,governance:governance,badges:badgeCounts,handoffs:Object.assign(handoffCounts,{averageAcknowledgementMinutes:ackSamples?Math.round(ackTotal/ackSamples):0,averageResolutionMinutes:resolutionSamples?Math.round(resolutionTotal/resolutionSamples):0})};
}
...
function leadershipWeeks_(){const local=parseDateSafe_(dateKey_(new Date())+'T12:00:00'),daysFromMonday=(local.getDay()+6)%7,currentStart=new Date(local.getTime()-daysFromMonday*86400000),currentEnd=new Date(currentStart.getTime()+6*86400000),nextStart=new Date(currentStart.getTime()+7*86400000),nextEnd=new Date(currentStart.getTime()+13*86400000);return{currentStart:dateKey_(currentStart),currentEnd:dateKey_(currentEnd),nextStart:dateKey_(nextStart),nextEnd:dateKey_(nextEnd)};}
function generateOperationalReport_(p,session){
  const type=String(p.reportType||'summary'),allowed=['summary','visitors','sponsor-performance','leadership-outlook','badges','handoffs','audit'];if(!allowed.includes(type))throw new Error('Select a valid VISTA report type.');
  ...
  if(type==='summary'){
    const data=getOperationalAnalytics_({days:range.days},session),s=data.summary,b=data.badges,h=data.handoffs;title='VISTA Operational Summary';columns=[{key:'category',label:'Category'},{key:'metric',label:'Metric'},{key:'value',label:'Value'}];rows=[
      {category:'Visitors',metric:'Total visits',value:s.totalVisits},{category:'Visitors',metric:'Approval rate',value:s.approvalRate+'%'},{category:'Visitors',metric:'Currently onsite',value:s.onsiteNow},{category:'Visitors',metric:'Average visit duration (minutes)',value:s.averageDurationMinutes},{category:'Visitors',metric:'Peak arrival hour',value:s.peakArrivalHour},{category:'Sponsor Governance',metric:'Advance-notice standard (days)',value:data.governance.advanceNoticeRequiredDays},{category:'Sponsor Governance',metric:'On-time requests',value:data.governance.compliantRequests},{category:'Sponsor Governance',metric:'Late / non-compliant requests',value:data.governance.lateRequests},{category:'Sponsor Governance',metric:'Compliance rate',value:data.governance.complianceRate+'%'},{category:'Sponsor Governance',metric:'Manual approvals',value:data.governance.manualApprovals},{category:'Sponsor Governance',metric:'Average approval response (minutes)',value:data.governance.averageResponseMinutes},{category:'Outcomes',metric:'Denied',value:s.denied},{category:'Outcomes',metric:'No show',value:s.noShow},{category:'Badges',metric:'Available',value:b.available},{category:'Badges',metric:'Issued',value:b.issued},{category:'Badges',metric:'Unavailable',value:b.unavailable},{category:'Handoffs',metric:'Pending',value:h.pending},{category:'Handoffs',metric:'Acknowledged',value:h.acknowledged},{category:'Handoffs',metric:'Resolved',value:h.resolved},{category:'Handoffs',metric:'Escalated',value:h.escalated},{category:'Handoffs',metric:'Average acknowledgement (minutes)',value:h.averageAcknowledgementMinutes},{category:'Handoffs',metric:'Average resolution (minutes)',value:h.averageResolutionMinutes}
    ];
  ...
  }else if(type==='sponsor-performance'){
    title='Sponsor & Approver Performance Scorecard';columns=[{key:'sponsor',label:'Sponsor / Approver'},{key:'email',label:'Email'},{key:'requests',label:'Requests'},{key:'compliant',label:'On-Time'},{key:'late',label:'Late'},{key:'complianceRate',label:'Compliance Rate'},{key:'approved',label:'Approved'},{key:'pending',label:'Pending'},{key:'denied',label:'Denied'},{key:'manual',label:'Manual Approvals'},{key:'response',label:'Avg. Response Minutes'},{key:'lead',label:'Avg. Lead Days'}];rows=sponsorPerformance_(readObjects_(FE.SHEETS.VISITS).filter(r=>inReportRange_(r.StartDate||r.CreatedAt,range))).map(g=>({sponsor:g.name,email:g.email,requests:g.totalRequests,compliant:g.compliantRequests,late:g.lateRequests,complianceRate:g.complianceRate+'%',approved:g.approvedRequests,pending:g.pendingRequests,denied:g.deniedRequests,manual:g.manualApprovals,response:g.averageResponseMinutes,lead:g.averageLeadDays}));
  }else if(type==='leadership-outlook'){
    const weeks=leadershipWeeks_();range.start=weeks.currentStart;range.end=weeks.nextEnd;title='Two-Week Visitor Leadership Outlook';columns=[{key:'week',label:'Planning Week'},{key:'date',label:'Visit Date'},{key:'arrival',label:'Arrival'},{key:'visitor',label:'Visitor'},{key:'company',label:'Company'},{key:'sponsor',label:'Sponsor / Approver'},{key:'status',label:'Status'},{key:'notice',label:'Advance Notice'},{key:'parking',label:'Parking Assignment'},{key:'office',label:'Office / Seating'},{key:'confirmation',label:'Confirmation'}];rows=readObjects_(FE.SHEETS.VISITS).filter(r=>{const d=dateKey_(r.StartDate);return d>=weeks.currentStart&&d<=weeks.nextEnd;}).map(r=>{const d=dateKey_(r.StartDate),notice=advanceNoticeForVisit_(r);return{week:d<=weeks.currentEnd?'Current Week '+weeks.currentStart+' – '+weeks.currentEnd:'Next Week '+weeks.nextStart+' – '+weeks.nextEnd,date:d,arrival:timeOnly_(r.ArrivalTime),visitor:String(r.FullName||''),company:String(r.Company||''),sponsor:String(r.SponsorName||''),status:String(r.Status||''),notice:notice.compliant?'Meets '+notice.requiredDays+'-day standard':'Late · '+notice.actualDays+' days',parking:String(r.ParkingLotAssignment||''),office:String(r.OfficeSeatingAssignment||''),confirmation:String(r.ConfirmationNumber||'')};});
  }else if(type==='badges'){
  ...
  if(query)rows=rows.filter(row=>Object.values(row).join(' ').toLowerCase().includes(query));rows=type==='leadership-outlook'?rows.sort((a,b)=>String(a.date).localeCompare(String(b.date))||String(a.arrival).localeCompare(String(b.arrival))):rows.sort((a,b)=>String(Object.values(b)[0]||'').localeCompare(String(Object.values(a)[0]||'')));const totalRows=rows.length,truncated=totalRows>limit;rows=rows.slice(0,limit);
...
function recordReportExport_(p,session){const type=String(p.reportType||''),format=String(p.format||'').toUpperCase();if(!['summary','visitors','sponsor-performance','leadership-outlook','badges','handoffs','audit'].includes(type))throw new Error('Invalid report type.');if(!['CSV','PRINT'].includes(format))throw new Error('Invalid report export format.');audit_(session,'REPORT_EXPORTED','','','Success',type+' · '+format+' · '+Number(p.rowCount||0)+' rows');return{ok:true};}
...
function notifySponsorVisitorArrival_(visit,badge,checkedInAt,p,session){
  const requested=Boolean(p&&p.sponsorNotified),recipients=[visit.OperationalSponsorEmail,visit.SponsorEmail].map(v=>String(v||'').trim()).filter(validEmailAddress_).filter((v,i,a)=>a.indexOf(v)===i),recipient=recipients.join(','),cfg=config_();
  ...
  if(!recipient){result.status='No Sponsor Email';result.error='The visitor request does not contain a primary or operational Sponsor email address.';return result;}
...
function notifyVisitorSponsorDecision_(visit,status,notes,session){
  ...
  const logistics=approved?`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:18px"><tr><td style="padding:13px 15px;background:#eaf4ff;border-radius:8px"><strong>Arrival assignments</strong><br>Parking: ${html_(visit.ParkingLotAssignment)}<br>Office / seating: ${html_(visit.OfficeSeatingAssignment)}</td></tr></table>`:'';
  const body=`<p style="margin:0 0 16px">Hello ${html_(visit.FullName||'Visitor')},</p><p style="margin:0 0 20px">The sponsor review for your Ford Energy visitor request is complete.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 20px"><tr><td style="padding:14px 16px;background:${background};border-left:5px solid ${color};border-radius:7px"><strong style="color:${color}">Decision: ${decision}</strong><br>${html_(summary)}</td></tr></table>${emailDetailsTable_(visit,`${dateOnly_(visit.StartDate)} ${timeOnly_(visit.ArrivalTime)} through ${dateOnly_(visit.EndDate)} ${timeOnly_(visit.DepartureTime)}`,[visit.VehicleYear,visit.VehicleMake,visit.VehicleModel,visit.LicensePlate].filter(Boolean).join(' ')||'Not provided')}${logistics}${note?`<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:18px"><tr><td style="padding:13px 15px;background:#f3f8fd;border-radius:8px"><strong>Sponsor note</strong><br>${html_(note)}</td></tr></table>`:''}${approved?emailButton_(config_().VGS_NAVIGATION_URL,'Open Visitor Navigation'):''}`;
  const textBody=`Hello ${visit.FullName||'Visitor'},\n\nDecision: ${decision}\n${summary}\n\nConfirmation: ${visit.ConfirmationNumber||''}\nVisit: ${dateOnly_(visit.StartDate)} ${timeOnly_(visit.ArrivalTime)}\nSponsor: ${visit.SponsorName||session.FullName||session.Username||''}${approved?'\nParking: '+String(visit.ParkingLotAssignment||'')+'\nOffice / seating: '+String(visit.OfficeSeatingAssignment||''):''}${note?'\nSponsor note: '+note:''}`;
...
function permissionsForRole_(role){
  ...
  if(r==='sponsor'||r==='approver')return {viewVisits:true,viewPhoto:true,approve:true,deny:true,checkIn:false,checkOut:false,noShow:false,admin:false,manageUsers:false,manageConfig:false,viewAudit:false,viewAnalytics:false,viewBadges:false,manageBadges:false,registerUnknownBadge:false,reportIncident:false,manageIncidents:false,manualVisitIntake:false,authorizeManualVisit:false,sponsorPortal:true,sponsorReports:true};
  if(r==='plant leader')return {viewVisits:true,viewPhoto:true,approve:false,deny:false,checkIn:false,checkOut:false,noShow:false,admin:false,manageUsers:false,manageConfig:false,viewAudit:false,viewAnalytics:false,viewBadges:false,manageBadges:false,registerUnknownBadge:false,reportIncident:false,manageIncidents:false,manualVisitIntake:false,authorizeManualVisit:false,sponsorPortal:true,sponsorReports:true};
  return none;
}
...
function sponsorRole_(session){const role=String(session&&session.Role||'').trim().toLowerCase();return role==='sponsor'||role==='approver'||role==='plant leader';}
...
function plantLeaderRole_(session){return String(session&&session.Role||'').trim().toLowerCase()==='plant leader';}
...
function activeSponsorLeadership_(){return readObjects_(FE.SHEETS.SPONSOR_HIERARCHY).filter(r=>String(r.Status||'Active')==='Active');}
function sponsorHierarchyAssignmentsForUser_(session){const userId=String(session&&session.UserID||'');return activeSponsorLeadership_().filter(r=>(String(r.LeaderUserID||'')===userId||String(r.OCMSponsorUserID||'')===userId)&&truthyActive_(r.ReportAccess));}
function leaderSponsorIdentities_(session){return sponsorHierarchyAssignmentsForUser_(session).map(r=>({Email:r.SponsorEmail,EmployeeID:r.SponsorEmployeeID,Username:r.SponsorUsername}));}
function sponsorHierarchyForVisit_(visit,hierarchies){const row=(hierarchies||activeSponsorLeadership_()).find(r=>sponsorIdentityMatchesVisit_(visit,{Email:r.SponsorEmail,EmployeeID:r.SponsorEmployeeID,Username:r.SponsorUsername}));return row?{hierarchyId:String(row.HierarchyID||''),plantLeader:{userId:String(row.LeaderUserID||''),name:String(row.LeaderName||''),email:String(row.LeaderEmail||''),level:String(row.LeaderLevel||'Plant Leadership')},ocmSponsor:{userId:String(row.OCMSponsorUserID||''),name:String(row.OCMSponsorName||''),email:String(row.OCMSponsorEmail||''),level:String(row.OCMSponsorLevel||'OCM Sponsor')},sponsor:{userId:String(row.SponsorUserID||''),name:String(row.SponsorName||visit.SponsorName||''),email:String(row.SponsorEmail||visit.SponsorEmail||'')},visitor:{name:String(visit.FullName||'')}}:null;}
function operationalSponsorMatchesVisit_(visit,user){const userId=String(user&&user.UserID||''),email=String(user&&user.Email||'').trim().toLowerCase(),employeeIds=[user&&user.EmployeeID,user&&user.Username].map(normalizeCDSID_).filter(Boolean);return Boolean((userId&&userId===String(visit&&visit.OperationalSponsorUserID||''))||(email&&email===String(visit&&visit.OperationalSponsorEmail||'').trim().toLowerCase())||employeeIds.includes(normalizeCDSID_(visit&&visit.OperationalSponsorEmployeeID)));}
function sponsorVisitAccess_(visit,session,delegations){
  ...
  if(plantLeaderRole_(session))return{allowed:true,type:'Leadership Oversight',delegatedFrom:String(visit.SponsorName||visit.SponsorEmail||'Assigned sponsor')};
  if(sponsorIdentityMatchesVisit_(visit,session))return{allowed:true,type:'Direct',delegatedFrom:''};
  if(operationalSponsorMatchesVisit_(visit,session))return{allowed:true,type:'Operational Sponsor',delegatedFrom:String(visit.SponsorName||visit.SponsorEmail||'Primary sponsor')};
  const hierarchy=sponsorHierarchyAssignmentsForUser_(session).find(r=>sponsorIdentityMatchesVisit_(visit,{Email:r.SponsorEmail,EmployeeID:r.SponsorEmployeeID,Username:r.SponsorUsername}));
  if(hierarchy)return{allowed:true,type:String(hierarchy.OCMSponsorUserID||'')===String(session.UserID||'')?'OCM Sponsor Oversight':'Leadership Oversight',delegatedFrom:String(visit.SponsorName||visit.SponsorEmail||'Assigned sponsor')};
...
function publicSponsorVisit_(row,session,delegations,hierarchies){const access=sponsorVisitAccess_(row,session,delegations);return Object.assign(publicVisit_(row),{sponsorAccess:access.type,delegatedFrom:access.delegatedFrom,delegationId:access.delegationId||'',sponsorHierarchy:sponsorHierarchyForVisit_(row,hierarchies),operationalSponsor:{userId:String(row.OperationalSponsorUserID||''),employeeId:String(row.OperationalSponsorEmployeeID||''),name:String(row.OperationalSponsorName||''),email:String(row.OperationalSponsorEmail||''),assignedAt:dateText_(row.OperationalSponsorAssignedAt),assignedBy:String(row.OperationalSponsorAssignedBy||'')}});}
function listVisitsForSession_(p,session){requirePermission_(session,'viewVisits');if(!sponsorRole_(session))return listVisits_(p);const delegations=activeSponsorDelegations_(new Date()),hierarchies=activeSponsorLeadership_(),visits=sponsorVisits_(p,session).map(r=>publicSponsorVisit_(r,session,delegations,hierarchies)),today=dateKey_(new Date());return{ok:true,visits:visits,kpis:{expectedToday:visits.filter(v=>v.startDate===today&&['Approved','Submitted','Pending Review','Pending Sponsor Review'].includes(v.status)).length,onsite:visits.filter(v=>v.status==='Checked In').length,checkedOutToday:visits.filter(v=>v.status==='Checked Out'&&String(v.checkOutTime||'').startsWith(today)).length,overdue:0}};}
function listSponsorDashboard_(p,session){
  ...
  const now=new Date(),delegations=activeSponsorDelegations_(now),hierarchies=activeSponsorLeadership_(),badgeRequests=latestPermanentBadgeRequestMap_(),rows=sponsorVisits_(p,session),visits=rows.map(r=>{const visit=publicSponsorVisit_(r,session,delegations,hierarchies),request=badgeRequests[String(r.VisitID||'')];visit.permanentBadgeRequest=request?publicPermanentBadgeRequest_(request):null;return visit;}),today=dateKey_(now),pending=visits.filter(v=>['Submitted','Pending Review','Pending Sponsor Review'].includes(v.status)).length,approved=visits.filter(v=>v.status==='Approved'&&v.startDate>=today).length,onsite=visits.filter(v=>v.status==='Checked In').length,completed=visits.filter(v=>v.status==='Checked Out').length,incoming=delegations.filter(r=>String(r.DelegateUserID||'')===String(session.UserID||'')).length,outgoing=delegations.filter(r=>String(r.SponsorUserID||'')===String(session.UserID||'')).length;
  const leadership=sponsorHierarchyAssignmentsForUser_(session),enterpriseOversight=sponsorOversightRole_(session)||plantLeaderRole_(session);
  return{ok:true,generatedAt:dateTime_(now),oversight:sponsorOversightRole_(session),enterpriseOversight:enterpriseOversight,leadershipOversight:enterpriseOversight||leadership.length>0,sponsor:{fullName:String(session.FullName||session.Username||''),email:String(session.Email||''),department:String(session.Department||''),role:String(session.Role||'')},summary:{pending:pending,approvedUpcoming:approved,onsite:onsite,completed:completed,total:visits.length,delegatedToMe:incoming,delegatedByMe:outgoing,assignedSponsors:enterpriseOversight?'All':leadership.length},visits:visits};
}
function publishEVChargingPolicy2026_2(){
  const sheet=ensureSheet_(SpreadsheetApp.getActive(),FE.SHEETS.EV_POLICIES,FE.EV_POLICY_HEADERS),rows=readObjects_(FE.SHEETS.EV_POLICIES),now=new Date(),version='2026.2',sitePolicy=[
    'Charging access is limited to approved Ford Energy and Ford Motor Company employees and the approved vehicle listed on the request.',
    'Use only designated employee EV charging spaces and compatible charging equipment. Do not use unauthorized adapters, extension cords, damaged cables, or equipment.',
    'Move the vehicle when charging is complete or when the 6-hour daily charging allotment has been reached so charging capacity remains available to other approved employees.',
    'Keep charging areas, access aisles, fire lanes, and accessible spaces clear. Follow posted time limits, parking controls, and Security instructions.',
  ...
  rows.forEach((row,index)=>{if(truthyActive_(row.Active))updateObjectRow_(FE.SHEETS.EV_POLICIES,index+2,{Active:'No',LastUpdatedAt:now});});
  const existing=rows.find(r=>String(r.Version||'')===version);if(existing){const index=rows.indexOf(existing);updateObjectRow_(FE.SHEETS.EV_POLICIES,index+2,{Title:'Ford Energy BlueOval Site Charging Policy and Terms of Use',EffectiveDate:now,SitePolicy:sitePolicy,StateAndUseTerms:stateTerms,Active:'Yes',ContentHash:sha256_([version,sitePolicy,stateTerms].join('|')),LastUpdatedAt:now});return 'EV Charging Policy '+version+' updated and activated with the 6-hour daily charging allotment.';}
  appendObjectRow_(sheet,{PolicyID:'EV-POLICY-'+version.replace(/[^A-Za-z0-9]/g,''),Title:'Ford Energy BlueOval Site Charging Policy and Terms of Use',Version:version,EffectiveDate:now,SitePolicy:sitePolicy,StateAndUseTerms:stateTerms,Active:'Yes',ContentHash:sha256_([version,sitePolicy,stateTerms].join('|')),LastUpdatedAt:now},FE.EV_POLICY_HEADERS);return 'EV Charging Policy '+version+' published with the 6-hour daily charging allotment.';
}
...
function operationalSponsorEligible_(user){return truthyActive_(user&&user.Active)&&Boolean(permissionsForRole_(user&&user.Role).sponsorPortal)&&Boolean(String(user&&user.Email||'').trim());}
function listOperationalSponsorDirectory_(p,session){const query=String(p.query||'').trim().toLowerCase(),rows=readObjects_(FE.SHEETS.USERS).filter(user=>operationalSponsorEligible_(user)&&String(user.UserID||'')!==String(session.UserID||'')).filter(user=>!query||[user.FullName,user.Email,user.EmployeeID,user.Department,user.Role].join(' ').toLowerCase().includes(query)).sort((a,b)=>String(a.FullName||'').localeCompare(String(b.FullName||''))).slice(0,250).map(publicSponsorDirectoryUser_);return{ok:true,users:rows};}
function listSponsorDelegations_(p,session){const now=new Date(),admin=sponsorOversightRole_(session),leadership=plantLeaderRole_(session),enterpriseView=admin||leadership,rows=readObjects_(FE.SHEETS.SPONSOR_DELEGATIONS).filter(r=>enterpriseView||String(r.SponsorUserID||'')===String(session.UserID||'')||String(r.DelegateUserID||'')===String(session.UserID||'')).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0)).slice(0,250).map(r=>publicSponsorDelegation_(r,now));return{ok:true,generatedAt:dateTime_(now),oversight:admin,leadershipView:leadership,delegations:rows,summary:{activeOutgoing:enterpriseView?rows.filter(r=>r.status==='Active').length:rows.filter(r=>r.sponsorUserId===String(session.UserID||'')&&r.status==='Active').length,activeIncoming:enterpriseView?0:rows.filter(r=>r.delegateUserId===String(session.UserID||'')&&r.status==='Active').length,scheduled:rows.filter(r=>r.status==='Scheduled').length}};}

function publicSponsorLeadership_(r){return{hierarchyId:String(r.HierarchyID||''),createdAt:dateText_(r.CreatedAt),sponsorUserId:String(r.SponsorUserID||''),sponsorName:String(r.SponsorName||''),sponsorEmail:String(r.SponsorEmail||''),ocmSponsorUserId:String(r.OCMSponsorUserID||''),ocmSponsorName:String(r.OCMSponsorName||''),ocmSponsorEmail:String(r.OCMSponsorEmail||''),ocmSponsorLevel:String(r.OCMSponsorLevel||'OCM Sponsor'),leaderUserId:String(r.LeaderUserID||''),leaderName:String(r.LeaderName||''),leaderEmail:String(r.LeaderEmail||''),leaderLevel:String(r.LeaderLevel||''),status:String(r.Status||''),notificationsEnabled:truthyActive_(r.NotificationsEnabled),reportAccess:truthyActive_(r.ReportAccess),notes:String(r.Notes||'')};}
function listSponsorLeadershipAuthorized_(p,session){const admin=sponsorOversightRole_(session),userId=String(session.UserID||''),rows=activeSponsorLeadership_().filter(r=>admin||String(r.SponsorUserID||'')===userId||String(r.OCMSponsorUserID||'')===userId||String(r.LeaderUserID||'')===userId).map(publicSponsorLeadership_);return{ok:true,oversight:admin,assignments:rows,canViewReports:admin||rows.some(r=>(r.leaderUserId===userId||r.ocmSponsorUserId===userId)&&r.reportAccess)};}
function saveSponsorLeadershipAuthorized_(p,session){validateRequired_(p,['sponsorUserId','ocmSponsorUserId','leaderUserId','leaderLevel']);const sponsor=activeSponsorUser_(p.sponsorUserId),ocm=activeSponsorUser_(p.ocmSponsorUserId),leader=activeSponsorUser_(p.leaderUserId),ids=[sponsor.UserID,ocm.UserID,leader.UserID].map(String);if(new Set(ids).size!==3)throw new Error('Plant Leadership, OCM Sponsor, and Primary Sponsor must be three different VISTA users.');const duplicate=activeSponsorLeadership_().find(r=>String(r.SponsorUserID)===String(sponsor.UserID));if(duplicate)throw new Error('This Sponsor already has an active leadership hierarchy. End the existing assignment before creating another.');const now=new Date(),record={HierarchyID:'SLH-'+Utilities.getUuid().slice(0,12).toUpperCase(),CreatedAt:now,SponsorUserID:sponsor.UserID,SponsorEmployeeID:sponsor.EmployeeID,SponsorUsername:sponsor.Username,SponsorName:sponsor.FullName,SponsorEmail:sponsor.Email,OCMSponsorUserID:ocm.UserID,OCMSponsorEmployeeID:ocm.EmployeeID,OCMSponsorUsername:ocm.Username,OCMSponsorName:ocm.FullName,OCMSponsorEmail:ocm.Email,OCMSponsorLevel:String(p.ocmSponsorLevel||'OCM Sponsor · LL5').trim().slice(0,60),LeaderUserID:leader.UserID,LeaderEmployeeID:leader.EmployeeID,LeaderUsername:leader.Username,LeaderName:leader.FullName,LeaderEmail:leader.Email,LeaderLevel:String(p.leaderLevel).trim().slice(0,40),Status:'Active',NotificationsEnabled:truthy_(p.notificationsEnabled===undefined?'Yes':p.notificationsEnabled)?'Yes':'No',ReportAccess:truthy_(p.reportAccess===undefined?'Yes':p.reportAccess)?'Yes':'No',Notes:String(p.notes||'').trim().slice(0,1000),CreatedByUserID:session.UserID,CreatedBy:session.FullName||session.Username,LastUpdatedAt:now};appendObjectRow_(SpreadsheetApp.getActive().getSheetByName(FE.SHEETS.SPONSOR_HIERARCHY),record,FE.SPONSOR_HIERARCHY_HEADERS);notifySponsorLeadershipAssignment_(record);audit_(session,'SPONSOR_LEADERSHIP_ASSIGNED','','','Success',record.LeaderName+' → '+record.OCMSponsorName+' → '+record.SponsorName);return{ok:true,assignment:publicSponsorLeadership_(record),message:'Four-level Sponsor leadership hierarchy created.'};}
function endSponsorLeadershipAuthorized_(p,session){validateRequired_(p,['hierarchyId']);const rows=readObjects_(FE.SHEETS.SPONSOR_HIERARCHY),index=rows.findIndex(r=>String(r.HierarchyID)===String(p.hierarchyId));if(index<0)throw new Error('Leadership assignment was not found.');const now=new Date();updateObjectRow_(FE.SHEETS.SPONSOR_HIERARCHY,index+2,{Status:'Ended',EndedAt:now,EndedByUserID:session.UserID,EndedBy:session.FullName||session.Username,LastUpdatedAt:now});audit_(session,'SPONSOR_LEADERSHIP_ENDED','','','Success',String(p.hierarchyId));return{ok:true,message:'Plant leadership oversight assignment ended.'};}
function notifySponsorLeadershipAssignment_(record){if(!truthyActive_(config_().SPONSOR_LEADERSHIP_EMAILS_ENABLED))return{sent:false,status:'Not requested'};const recipients=[record.LeaderEmail,record.OCMSponsorEmail].filter(validEmailAddress_).filter((email,index,array)=>array.indexOf(email)===index);if(!recipients.length)return{sent:false,status:'No valid recipients'};try{const portal=config_().SPONSOR_PORTAL_URL||'https://kyblueoval.github.io/Ford-Energy-VISTA/sponsor-portal/',body=`<p>A governed VISTA Sponsor hierarchy has been assigned:</p><p style="padding:14px 16px;background:#eef6ff;border-left:5px solid #0b67ad"><strong>${html_(record.LeaderName)}</strong> · ${html_(record.LeaderLevel)}<br>↓<br><strong>${html_(record.OCMSponsorName)}</strong> · ${html_(record.OCMSponsorLevel)}<br>↓<br><strong>${html_(record.SponsorName)}</strong> · Primary Sponsor</p><p>Plant Leadership and the OCM Sponsor can review Sponsor performance and the current/next-week visitor outlook. Visitor actions remain attributed to the person who performs them.</p>${emailButton_(portal,'Open Sponsor Oversight')}`;MailApp.sendEmail({to:recipients.join(','),subject:'VISTA Sponsor hierarchy assigned',body:`VISTA hierarchy assigned for ${record.SponsorName}. Open ${portal}`,htmlBody:brandedEmail_({title:'Sponsor Hierarchy Assigned',kicker:'FORD ENERGY · VISTA LEADERSHIP',body:body})});return{sent:true,status:'Sent'};}catch(e){return{sent:false,status:'Failed',error:String(e&&e.message||e)};}}

function assignmentUser_(userId){const found=findUserById_(userId),user=found.obj;if(!operationalSponsorEligible_(user))throw new Error('Select an active salaried VISTA user with Sponsor Portal access and an email address.');return user;}
function assignOperationalSponsorAuthorized_(p,session){validateRequired_(p,['visitId','operationalSponsorUserId','reason']);const found=requireSponsorOwnedVisit_(p.visitId,session),visit=found.obj,access=sponsorVisitAccess_(visit,session);if(!sponsorOversightRole_(session)&&access.type!=='Direct')throw new Error('Only the primary Sponsor or an Administrator can assign operational responsibility.');const delegate=assignmentUser_(p.operationalSponsorUserId);if(sponsorIdentityMatchesVisit_(visit,delegate))throw new Error('The primary Sponsor already owns this visitor request.');const now=new Date(),old=readObjects_(FE.SHEETS.VISIT_RESPONSIBILITY).filter(r=>String(r.VisitID)===String(visit.VisitID)&&String(r.Status)==='Active');old.forEach(r=>{const rows=readObjects_(FE.SHEETS.VISIT_RESPONSIBILITY),index=rows.findIndex(x=>String(x.AssignmentID)===String(r.AssignmentID));if(index>=0)updateObjectRow_(FE.SHEETS.VISIT_RESPONSIBILITY,index+2,{Status:'Superseded',EndedAt:now,EndedByUserID:session.UserID,EndedBy:session.FullName||session.Username,LastUpdatedAt:now});});const primary=readObjects_(FE.SHEETS.USERS).find(u=>sponsorIdentityMatchesVisit_(visit,u))||{},record={AssignmentID:'VRA-'+Utilities.getUuid().slice(0,12).toUpperCase(),VisitID:visit.VisitID,ConfirmationNumber:visit.ConfirmationNumber,CreatedAt:now,PrimarySponsorUserID:primary.UserID||'',PrimarySponsorName:visit.SponsorName,PrimarySponsorEmail:visit.SponsorEmail,OperationalSponsorUserID:delegate.UserID,OperationalSponsorEmployeeID:delegate.EmployeeID,OperationalSponsorName:delegate.FullName,OperationalSponsorEmail:delegate.Email,Status:'Active',Reason:String(p.reason).trim().slice(0,1000),AssignedByUserID:session.UserID,AssignedBy:session.FullName||session.Username,LastUpdatedAt:now};appendObjectRow_(SpreadsheetApp.getActive().getSheetByName(FE.SHEETS.VISIT_RESPONSIBILITY),record,FE.VISIT_RESPONSIBILITY_HEADERS);updateObjectRow_(FE.SHEETS.VISITS,found.row,{OperationalSponsorUserID:delegate.UserID,OperationalSponsorEmployeeID:delegate.EmployeeID,OperationalSponsorName:delegate.FullName,OperationalSponsorEmail:delegate.Email,OperationalSponsorAssignedAt:now,OperationalSponsorAssignedBy:record.AssignedBy,LastUpdatedAt:now});notifyOperationalSponsorAssignment_(record,visit);logActivity_(visit.VisitID,'OPERATIONAL_SPONSOR_ASSIGNED',record.AssignedBy,'',delegate.FullName+' · '+record.Reason);audit_(session,'VISIT_OPERATIONAL_SPONSOR_ASSIGNED',visit.VisitID,visit.ConfirmationNumber,'Success',delegate.FullName+' · '+delegate.Email);return{ok:true,message:delegate.FullName+' is now the operational Sponsor for this visitor.'};}
function clearOperationalSponsorAuthorized_(p,session){validateRequired_(p,['visitId']);const found=requireSponsorOwnedVisit_(p.visitId,session),visit=found.obj,access=sponsorVisitAccess_(visit,session);if(!sponsorOversightRole_(session)&&access.type!=='Direct')throw new Error('Only the primary Sponsor or an Administrator can clear operational responsibility.');const now=new Date(),rows=readObjects_(FE.SHEETS.VISIT_RESPONSIBILITY),record=rows.find(r=>String(r.VisitID)===String(visit.VisitID)&&String(r.Status)==='Active');if(record){const index=rows.findIndex(r=>String(r.AssignmentID)===String(record.AssignmentID));updateObjectRow_(FE.SHEETS.VISIT_RESPONSIBILITY,index+2,{Status:'Ended',EndedAt:now,EndedByUserID:session.UserID,EndedBy:session.FullName||session.Username,LastUpdatedAt:now});}updateObjectRow_(FE.SHEETS.VISITS,found.row,{OperationalSponsorUserID:'',OperationalSponsorEmployeeID:'',OperationalSponsorName:'',OperationalSponsorEmail:'',OperationalSponsorAssignedAt:'',OperationalSponsorAssignedBy:'',LastUpdatedAt:now});logActivity_(visit.VisitID,'OPERATIONAL_SPONSOR_CLEARED',session.FullName||session.Username,'','Primary Sponsor resumed direct responsibility');audit_(session,'VISIT_OPERATIONAL_SPONSOR_CLEARED',visit.VisitID,visit.ConfirmationNumber,'Success','Primary Sponsor resumed responsibility');return{ok:true,message:'Operational responsibility returned to the primary Sponsor.'};}
function notifyOperationalSponsorAssignment_(record,visit){if(!truthyActive_(config_().SPONSOR_LEADERSHIP_EMAILS_ENABLED))return{sent:false,status:'Disabled'};const recipients=[record.OperationalSponsorEmail,record.PrimarySponsorEmail].filter(validEmailAddress_).filter((v,i,a)=>a.indexOf(v)===i);if(!recipients.length)return{sent:false,status:'No recipients'};try{const portal=config_().SPONSOR_PORTAL_URL||'https://kyblueoval.github.io/Ford-Energy-VISTA/sponsor-portal/',body=`<p><strong>${html_(record.OperationalSponsorName)}</strong> is now the operational Sponsor for ${html_(visit.FullName)}.</p>${emailDetailsTable_(visit,`${dateOnly_(visit.StartDate)} ${timeOnly_(visit.ArrivalTime)} through ${dateOnly_(visit.EndDate)} ${timeOnly_(visit.DepartureTime)}`,[visit.VehicleMake,visit.VehicleModel,visit.LicensePlate].filter(Boolean).join(' ')||'Not provided')}<p><strong>Reason:</strong> ${html_(record.Reason)}</p>${emailButton_(portal,'Open Visitor Responsibility')}`;MailApp.sendEmail({to:recipients.join(','),subject:`VISTA visitor responsibility assigned: ${visit.FullName}`,body:`${record.OperationalSponsorName} is now responsible for ${visit.FullName}. ${portal}`,htmlBody:brandedEmail_({title:'Visitor Responsibility Assigned',kicker:'FORD ENERGY · VISTA SPONSOR COORDINATION',body:body})});return{sent:true,status:'Sent',recipients:recipients};}catch(e){return{sent:false,status:'Failed',error:String(e&&e.message||e)};}}

function getLeadershipReportsAuthorized_(p,session){const enterpriseView=sponsorOversightRole_(session)||plantLeaderRole_(session),identities=enterpriseView?[]:leaderSponsorIdentities_(session);if(!enterpriseView&&!identities.length)throw new Error('No active Plant Leadership reporting assignment is linked to this account.');const all=readObjects_(FE.SHEETS.VISITS),scoped=enterpriseView?all:all.filter(v=>identities.some(identity=>sponsorIdentityMatchesVisit_(v,identity))),performance=enterpriseView?sponsorPerformancePopulation_(scoped):sponsorPerformance_(scoped),weeks=leadershipWeeks_(),outlook=scoped.filter(v=>{const d=dateKey_(v.StartDate);return d>=weeks.currentStart&&d<=weeks.nextEnd;}).map(v=>{const notice=advanceNoticeForVisit_(v),d=dateKey_(v.StartDate);return{week:d<=weeks.currentEnd?'Current Week':'Next Week',date:d,arrival:timeOnly_(v.ArrivalTime),visitor:String(v.FullName||''),company:String(v.Company||''),sponsor:String(v.SponsorName||''),operationalSponsor:String(v.OperationalSponsorName||''),status:String(v.Status||''),advanceNotice:notice.compliant?'Compliant':'Late',parking:String(v.ParkingLotAssignment||''),office:String(v.OfficeSeatingAssignment||'')}}).sort((a,b)=>a.date.localeCompare(b.date)||a.arrival.localeCompare(b.arrival));return{ok:true,generatedAt:dateTime_(new Date()),enterpriseView:enterpriseView,weeks:weeks,performance:performance,outlook:outlook,summary:{assignedSponsors:performance.length,totalVisits:scoped.length,currentWeek:outlook.filter(r=>r.week==='Current Week').length,nextWeek:outlook.filter(r=>r.week==='Next Week').length,lateRequests:performance.reduce((n,r)=>n+r.lateRequests,0)}};}
function sendSponsorLeadershipReportsAuthorized_(p,session){const result=sendSponsorLeadershipReports_();audit_(session,'SPONSOR_LEADERSHIP_REPORTS_SENT','','','Success',result.sent+' sent · '+result.failed+' failed');return Object.assign({ok:true},result);}
function sendSponsorLeadershipReports_(){if(!truthyActive_(config_().SPONSOR_LEADERSHIP_REPORTS_ENABLED))return{sent:0,failed:0,message:'Leadership report emails are disabled.'};const leaders=activeSponsorLeadership_().filter(r=>truthyActive_(r.ReportAccess)&&truthyActive_(r.NotificationsEnabled)).reduce((map,r)=>(map[String(r.LeaderUserID||r.LeaderEmail)]=r,map),{}),portal=config_().SPONSOR_PORTAL_URL||'https://kyblueoval.github.io/Ford-Energy-VISTA/sponsor-portal/';let sent=0,failed=0;Object.values(leaders).forEach(r=>{try{const user=findUserById_(r.LeaderUserID).obj,data=getLeadershipReportsAuthorized_({},user),performanceRows=(data.performance||[]).slice(0,12).map((item,index)=>`<tr><td style="padding:8px;border-bottom:1px solid #d9e5f2">${index+1}</td><td style="padding:8px;border-bottom:1px solid #d9e5f2"><strong>${html_(item.name)}</strong></td><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.totalRequests)}</td><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.complianceRate)}%</td><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.averageResponseMinutes)} min</td></tr>`).join(''),outlookRows=(data.outlook||[]).slice(0,20).map(item=>`<tr><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.week)} · ${html_(item.date)} ${html_(item.arrival)}</td><td style="padding:8px;border-bottom:1px solid #d9e5f2"><strong>${html_(item.visitor)}</strong></td><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.sponsor)}</td><td style="padding:8px;border-bottom:1px solid #d9e5f2">${html_(item.status)} · ${html_(item.advanceNotice)}</td></tr>`).join(''),body=`<p>Hello ${html_(r.LeaderName)},</p><p>Your VISTA Plant Leadership snapshot is ready.</p><table role="presentation" style="width:100%;border-collapse:collapse;margin:16px 0"><tr><td style="padding:12px;background:#eef6ff"><strong>${data.summary.assignedSponsors}</strong><br>Assigned Sponsors</td><td style="padding:12px;background:#eef6ff"><strong>${data.summary.currentWeek}</strong><br>Current Week</td><td style="padding:12px;background:#eef6ff"><strong>${data.summary.nextWeek}</strong><br>Next Week</td><td style="padding:12px;background:#fff3e8"><strong>${data.summary.lateRequests}</strong><br>Late Requests</td></tr></table><h3 style="color:#0b3f82">Sponsor Performance Standings</h3><table role="presentation" style="width:100%;border-collapse:collapse"><tr style="background:#0b3f82;color:#fff"><th style="padding:8px">Rank</th><th style="padding:8px">Sponsor</th><th style="padding:8px">Requests</th><th style="padding:8px">Advance Compliance</th><th style="padding:8px">Avg. Response</th></tr>${performanceRows||'<tr><td colspan="5" style="padding:12px">No Sponsor activity in the reporting window.</td></tr>'}</table><h3 style="color:#0b3f82">Current + Next Week Outlook</h3><table role="presentation" style="width:100%;border-collapse:collapse"><tr style="background:#0b3f82;color:#fff"><th style="padding:8px">Date</th><th style="padding:8px">Visitor</th><th style="padding:8px">Sponsor</th><th style="padding:8px">Status</th></tr>${outlookRows||'<tr><td colspan="4" style="padding:12px">No visits scheduled.</td></tr>'}</table>${emailButton_(portal,'Open Executive Sponsor Reports')}`;MailApp.sendEmail({to:r.LeaderEmail,subject:'VISTA weekly Sponsor performance and two-week outlook',body:`Your VISTA leadership report is ready: ${portal}`,htmlBody:brandedEmail_({title:'Sponsor Leadership Outlook',kicker:'FORD ENERGY · VISTA EXECUTIVE REPORTING',body:body})});sent++;}catch(e){failed++;}});return{sent:sent,failed:failed,message:sent+' leadership report email(s) sent.'};}
function installSponsorLeadershipReportTrigger(){ScriptApp.getProjectTriggers().filter(t=>t.getHandlerFunction()==='sendSponsorLeadershipReports_').forEach(t=>ScriptApp.deleteTrigger(t));const cfg=config_(),day=String(cfg.SPONSOR_LEADERSHIP_REPORT_DAY||'MONDAY').toUpperCase(),days={SUNDAY:ScriptApp.WeekDay.SUNDAY,MONDAY:ScriptApp.WeekDay.MONDAY,TUESDAY:ScriptApp.WeekDay.TUESDAY,WEDNESDAY:ScriptApp.WeekDay.WEDNESDAY,THURSDAY:ScriptApp.WeekDay.THURSDAY,FRIDAY:ScriptApp.WeekDay.FRIDAY,SATURDAY:ScriptApp.WeekDay.SATURDAY},weekDay=days[day]||ScriptApp.WeekDay.MONDAY,hour=Math.max(0,Math.min(23,Number(cfg.SPONSOR_LEADERSHIP_REPORT_HOUR||6)));ScriptApp.newTrigger('sendSponsorLeadershipReports_').timeBased().onWeekDay(weekDay).atHour(hour).create();return 'Weekly Sponsor Leadership report trigger installed for '+day+' at '+hour+':00.';}

function publicPermanentBadgeRequest_(r){return{requestId:String(r.RequestID||''),createdAt:dateText_(r.CreatedAt),status:String(r.Status||''),visitId:String(r.VisitID||''),confirmationNumber:String(r.ConfirmationNumber||''),visitorName:String(r.VisitorName||''),visitorEmail:String(r.VisitorEmail||''),company:String(r.Company||''),hasPhoto:Boolean(r.VisitorPhotoFileId||r.VisitorPhotoFileName||r.VisitorPhotoUrl),badgeType:String(r.BadgeType||''),accessRequired:String(r.AccessRequired||''),visitPattern:String(r.VisitPattern||''),businessJustification:String(r.BusinessJustification||''),sponsorName:String(r.SponsorName||''),sponsorEmail:String(r.SponsorEmail||''),sponsorRole:String(r.SponsorRole||''),sponsorAccessType:String(r.SponsorAccessType||''),onboardingIntakeUrl:String(r.OnboardingIntakeUrl||''),visitorNotificationStatus:String(r.VisitorNotificationStatus||''),securityNotificationStatus:String(r.SecurityNotificationStatus||''),securityNotificationRecipients:String(r.SecurityNotificationRecipients||''),processingNotes:String(r.ProcessingNotes||''),issuedBadgeNumber:String(r.IssuedBadgeNumber||''),issuedBadgeUid:String(r.IssuedBadgeUID||''),completedAt:dateText_(r.CompletedAt),completedBy:String(r.CompletedBy||'')};}
function latestPermanentBadgeRequestMap_(){return readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS).reduce((map,r)=>{const key=String(r.VisitID||''),current=map[key];if(!current||new Date(r.CreatedAt||0)>new Date(current.CreatedAt||0))map[key]=r;return map;},{});}
function findPermanentBadgeRequest_(requestId){const rows=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS),index=rows.findIndex(r=>String(r.RequestID||'')===String(requestId||''));if(index<0)throw new Error('Permanent badge request was not found.');return{row:index+2,obj:rows[index]};}
function permanentBadgeRequestDetails_(record){const rows=[['Visitor',`${html_(record.VisitorName)}<br><span style="color:#667587">${html_(record.Company)}</span>`],['Badge type',html_(record.BadgeType)],['Access required',html_(record.AccessRequired)],['Expected duration / frequency',html_(record.VisitPattern)],['Business justification',html_(record.BusinessJustification)],['Authorized by',`${html_(record.SponsorName)}<br><span style="color:#667587">${html_(record.SponsorRole)}</span>`],['Visitor confirmation',html_(record.ConfirmationNumber)],['Badge request',`<strong style="color:#003478">${html_(record.RequestID)}</strong>`]];return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;border:1px solid #dce6f0;border-radius:10px;border-collapse:separate;overflow:hidden">${rows.map((x,i)=>`<tr><td width="34%" valign="top" style="padding:11px 13px;background:${i%2===0?'#f6f9fc':'#ffffff'};border-bottom:${i===rows.length-1?'0':'1px solid #e2eaf2'};font-size:12px;line-height:18px;text-transform:uppercase;letter-spacing:.55px;font-weight:bold;color:#667587">${x[0]}</td><td valign="top" style="padding:11px 13px;background:${i%2===0?'#f6f9fc':'#ffffff'};border-bottom:${i===rows.length-1?'0':'1px solid #e2eaf2'};font-size:14px;line-height:20px;color:#16283d">${x[1]}</td></tr>`).join('')}</table>`;}
function notifyPermanentBadgeAuthorization_(record,visit){
  const cfg=config_(),intakeUrl=String(record.OnboardingIntakeUrl||cfg.PERMANENT_BADGE_INTAKE_URL||'').trim(),qrUrl=String(cfg.PERMANENT_BADGE_QR_URL||'').trim(),visitorEnabled=truthyActive_(String(cfg.PERMANENT_BADGE_VISITOR_EMAIL_ENABLED||'YES')),securityEnabled=truthyActive_(String(cfg.PERMANENT_BADGE_SECURITY_EMAIL_ENABLED||'NO')),visitor={requested:visitorEnabled,sent:false,status:visitorEnabled?'Pending':'Disabled',recipient:String(record.VisitorEmail||'').trim(),sentAt:'',error:''},parsed=facilitiesRecipients_(cfg.PERMANENT_BADGE_SECURITY_EMAILS),security={requested:securityEnabled,sent:false,status:securityEnabled?'Pending':'Disabled',recipients:parsed.emails.join(', '),sentAt:'',error:''},details=permanentBadgeRequestDetails_(record);
  if(visitorEnabled){
    if(!validEmailAddress_(visitor.recipient)){visitor.status='Invalid Visitor Email';visitor.error='The linked visitor record does not contain a valid email address.';}
    else if(!intakeUrl){visitor.status='Missing Intake URL';visitor.error='PERMANENT_BADGE_INTAKE_URL is blank in Config.';}
    else try{
      const qr=qrUrl?`<div style="text-align:center;margin:22px 0"><a href="${html_(intakeUrl)}"><img src="${html_(qrUrl)}" width="230" alt="Open Ford Energy badge intake" style="display:block;width:230px;max-width:70%;height:auto;margin:0 auto;border:1px solid #dce6f0;border-radius:12px"></a><div style="font-size:12px;color:#667587;margin-top:8px">Scan or select the button below to begin badge intake.</div></div>`:'';
      const body=`<p style="margin:0 0 16px">Hello ${html_(record.VisitorName)},</p><p style="margin:0 0 20px">Your Ford Energy sponsor has authorized a request for a permanent site badge. Complete the standard Ford Energy Onboarding Badge Request Intake so Global Security can review and produce the credential.</p><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin:0 0 20px"><tr><td style="padding:14px 16px;background:#eaf4ff;border-left:5px solid #0b67ad;border-radius:7px"><strong style="color:#003478">Authorized badge:</strong> ${html_(record.BadgeType)}<br><strong style="color:#003478">Requested access:</strong> ${html_(record.AccessRequired)}</td></tr></table>${details}${qr}${emailButton_(intakeUrl,'Complete Ford Energy Badge Intake')}<p style="margin:18px 0 0;color:#516477">Authorization does not activate a badge automatically. Global Security must verify the submitted intake and issue the physical credential.</p>`;
      MailApp.sendEmail({to:visitor.recipient,subject:`Permanent Ford Energy badge authorized (${record.RequestID})`,body:`Your sponsor authorized a ${record.BadgeType} badge request. Complete the Ford Energy badge intake: ${intakeUrl}`,htmlBody:brandedEmail_({title:'Permanent Badge Authorized',kicker:'FORD ENERGY · VISTA SPONSOR AUTHORIZATION',preheader:`Complete badge intake for request ${record.RequestID}`,body:body,footerNote:'This sponsor authorization and email delivery are retained in the VISTA audit record.'})});visitor.sent=true;visitor.status='Sent';visitor.sentAt=new Date();
    }catch(err){visitor.status='Failed';visitor.error=String(err&&err.message||err).replace(/\s+/g,' ').slice(0,500);}
  }
  if(securityEnabled){
    if(parsed.invalid.length){security.status='Invalid Recipients';security.error='Invalid Global Security email address: '+parsed.invalid.join(', ');}
    else if(!parsed.emails.length){security.status='No Recipients';security.error='PERMANENT_BADGE_SECURITY_EMAILS is blank.';}
    else try{
      let photoHtml='<p style="padding:14px 16px;background:#fff4dc;border-left:5px solid #ff7a00;border-radius:7px"><strong>Visitor photograph:</strong> Not available. Verify the VISTA visitor record before production.</p>',inlineImages={};
      try{const file=resolveVisitPhotoFile_(visit);if(file){const blob=file.getBlob().setName(String(record.VisitorPhotoFileName||file.getName()||'visitor-photo.jpg'));inlineImages.visitorPhoto=blob;photoHtml=`<div style="text-align:center;margin:0 0 20px"><img src="cid:visitorPhoto" width="220" alt="${html_(record.VisitorName)} visitor photograph" style="display:block;width:220px;max-width:55%;height:auto;margin:0 auto;border:5px solid #e5edf5;border-radius:14px"><div style="font-size:13px;color:#516477;margin-top:8px">${html_(record.VisitorName)} · ${html_(record.Company)}</div></div>`;}}catch(photoError){photoHtml='<p style="padding:14px 16px;background:#fff4dc;border-left:5px solid #ff7a00;border-radius:7px"><strong>Visitor photograph:</strong> Stored photo could not be loaded. Review the linked VISTA visitor record.</p>';}
      const body=`<p style="margin:0 0 20px">A VISTA Sponsor or Approver authorized permanent badge issuance for the visitor below. Review the linked identity information and standard badge intake before printing or activating a credential.</p>${photoHtml}${details}${intakeUrl?emailButton_(intakeUrl,'Open Standard Badge Intake'):''}`;
      const message={to:parsed.emails.join(','),subject:`Permanent badge authorization: ${record.VisitorName} · ${record.BadgeType}`,body:`Permanent badge authorization for ${record.VisitorName}, ${record.Company}. Type: ${record.BadgeType}. Access: ${record.AccessRequired}. Request: ${record.RequestID}.`,htmlBody:brandedEmail_({title:'Permanent Badge Authorization',kicker:'FORD ENERGY · GLOBAL SECURITY',preheader:`${record.BadgeType} request for ${record.VisitorName}`,body:body,footerNote:'The sponsor identity, visitor record, delivery result, and subsequent processing status are retained in VISTA.'})};if(Object.keys(inlineImages).length)message.inlineImages=inlineImages;MailApp.sendEmail(message);security.sent=true;security.status='Sent';security.sentAt=new Date();
    }catch(err){security.status='Failed';security.error=String(err&&err.message||err).replace(/\s+/g,' ').slice(0,500);}
  }
  return{visitor:visitor,security:security};
}
function createPermanentBadgeRequestAuthorized_(p,session){
  requirePermission_(session,'approve');validateRequired_(p,['visitId','badgeType','accessRequired','visitPattern','businessJustification']);const row=requireSponsorOwnedVisit_(p.visitId,session),visit=row.obj,status=String(visit.Status||''),allowedTypes=['Ford Energy (Blue)','Ford Energy Contractor (Green)','Ford Energy OEM (Orange)'];
  if(!['Approved','Checked In','Checked Out'].includes(status))throw new Error('Permanent badge authorization is available only after the visitor request has been approved.');if(!allowedTypes.includes(String(p.badgeType)))throw new Error('Select an approved permanent badge type.');if(!validEmailAddress_(visit.Email))throw new Error('The visitor record must contain a valid email address before badge authorization.');if(!(visit.PhotoFileId||visit.PhotoFileName||visit.PhotoUrl))throw new Error('The visitor must complete photograph intake before a permanent badge can be authorized.');
  const activeStatuses=['Pending Security Review','Approved for Production','Returned for Correction','In Production','Ready for Pickup','Issued'],duplicate=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS).find(r=>activeStatuses.includes(String(r.Status||''))&&(String(r.VisitID||'')===String(visit.VisitID||'')||String(r.VisitorEmail||'').trim().toLowerCase()===String(visit.Email||'').trim().toLowerCase()));if(duplicate)throw new Error('An active permanent badge request already exists for this visitor: '+String(duplicate.RequestID||''));
  const now=new Date(),access=sponsorVisitAccess_(visit,session),cfg=config_(),record={RequestID:'PBR-'+Utilities.getUuid().slice(0,12).toUpperCase(),CreatedAt:now,Status:'Pending Security Review',VisitID:String(visit.VisitID||''),ConfirmationNumber:String(visit.ConfirmationNumber||''),VisitorName:String(visit.FullName||''),VisitorEmail:String(visit.Email||''),Company:String(visit.Company||''),VisitorPhotoFileId:String(visit.PhotoFileId||''),VisitorPhotoFileName:String(visit.PhotoFileName||''),VisitorPhotoUrl:String(visit.PhotoUrl||''),BadgeType:String(p.badgeType),AccessRequired:String(p.accessRequired).trim().slice(0,2000),VisitPattern:String(p.visitPattern).trim().slice(0,1000),BusinessJustification:String(p.businessJustification).trim().slice(0,2000),SponsorUserID:String(session.UserID||''),SponsorName:String(session.FullName||session.Username||''),SponsorEmail:String(session.Email||''),SponsorRole:String(session.Role||''),SponsorAccessType:String(access.type||''),OnboardingIntakeUrl:String(cfg.PERMANENT_BADGE_INTAKE_URL||''),VisitorNotificationSent:'No',VisitorNotificationStatus:'Pending',SecurityNotificationSent:'No',SecurityNotificationStatus:'Pending',LastUpdatedAt:now};
  const sheet=SpreadsheetApp.getActive().getSheetByName(FE.SHEETS.PERMANENT_BADGE_REQUESTS);appendObjectRow_(sheet,record,FE.PERMANENT_BADGE_REQUEST_HEADERS);const found=findPermanentBadgeRequest_(record.RequestID),delivery=notifyPermanentBadgeAuthorization_(record,visit),updates={VisitorNotificationSent:delivery.visitor.sent?'Yes':'No',VisitorNotificationStatus:delivery.visitor.status,VisitorNotificationAt:delivery.visitor.sent?delivery.visitor.sentAt:'',VisitorNotificationError:delivery.visitor.error,SecurityNotificationSent:delivery.security.sent?'Yes':'No',SecurityNotificationStatus:delivery.security.status,SecurityNotificationRecipients:delivery.security.recipients,SecurityNotificationAt:delivery.security.sent?delivery.security.sentAt:'',SecurityNotificationError:delivery.security.error,LastUpdatedAt:new Date()};updateObjectRow_(FE.SHEETS.PERMANENT_BADGE_REQUESTS,found.row,updates);Object.assign(record,updates);
  const details=[record.BadgeType,record.AccessRequired,'Visitor email '+delivery.visitor.status,'Global Security email '+delivery.security.status].join(' · ');logActivity_(visit.VisitID,'PERMANENT_BADGE_AUTHORIZED',record.SponsorName,'',record.RequestID+' · '+details);audit_(session,'SPONSOR_PERMANENT_BADGE_AUTHORIZED',visit.VisitID,visit.ConfirmationNumber,delivery.security.requested&&!delivery.security.sent?'Warning':'Success',record.RequestID+' · '+details);return{ok:true,message:'Permanent badge request authorized and recorded.',request:publicPermanentBadgeRequest_(record),delivery:delivery};
}
function listPermanentBadgeRequestsAuthorized_(p,session){const query=String(p.query||'').trim().toLowerCase(),status=String(p.status||'').trim();let rows=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS);rows=rows.filter(r=>!status||String(r.Status||'')===status||(status==='Pending Security Review'&&String(r.Status||'')==='Authorized - Pending Security Processing')).filter(r=>!query||[r.RequestID,r.VisitorName,r.VisitorEmail,r.Company,r.BadgeType,r.AccessRequired,r.SponsorName,r.ConfirmationNumber,r.IssuedBadgeNumber,r.IssuedBadgeUID].join(' ').toLowerCase().includes(query)).sort((a,b)=>new Date(b.CreatedAt||0)-new Date(a.CreatedAt||0));const all=readObjects_(FE.SHEETS.PERMANENT_BADGE_REQUESTS),count=s=>all.filter(r=>String(r.Status||'')===s).length;return{ok:true,generatedAt:dateTime_(new Date()),summary:{total:all.length,pending:count('Pending Security Review')+count('Authorized - Pending Security Processing'),production:count('Approved for Production')+count('In Production')+count('Ready for Pickup'),issued:count('Issued'),closed:count('Closed')+count('Denied')+count('Cancelled')},requests:rows.slice(0,500).map(publicPermanentBadgeRequest_)};}
function updatePermanentBadgeRequestStatusAuthorized_(p,session){validateRequired_(p,['requestId','status']);const allowed=['Pending Security Review','Approved for Production','Returned for Correction','In Production','Ready for Pickup','Issued','Denied','Closed','Cancelled'],status=String(p.status);if(!allowed.includes(status))throw new Error('Select a valid permanent badge processing status.');if(['Returned for Correction','Denied'].includes(status)&&!String(p.processingNotes||'').trim())throw new Error('Processing notes are required when returning or denying a badge request.');if(status==='Issued'&&!String(p.issuedBadgeNumber||'').trim())throw new Error('Enter the issued badge number before marking this request Issued.');const found=findPermanentBadgeRequest_(p.requestId),now=new Date(),updates={Status:status,ProcessingNotes:String(p.processingNotes||'').trim().slice(0,2000),IssuedBadgeNumber:String(p.issuedBadgeNumber||found.obj.IssuedBadgeNumber||'').trim(),IssuedBadgeUID:String(p.issuedBadgeUid||found.obj.IssuedBadgeUID||'').trim(),LastUpdatedAt:now};if(['Issued','Denied','Closed','Cancelled'].includes(status)){updates.CompletedAt=now;updates.CompletedByUserID=String(session.UserID||'');updates.CompletedBy=String(session.FullName||session.Username||'');}updateObjectRow_(FE.SHEETS.PERMANENT_BADGE_REQUESTS,found.row,updates);const updated=Object.assign({},found.obj,updates),details=[found.obj.RequestID,status,updates.IssuedBadgeNumber?'Badge '+updates.IssuedBadgeNumber:'',updates.ProcessingNotes].filter(Boolean).join(' · '),notification=notifyPermanentBadgeStatus_(updated),auditStatus=notification.status==='Failed'?'Warning':'Success';logActivity_(found.obj.VisitID,'PERMANENT_BADGE_STATUS_UPDATED',session.FullName||session.Username,'',details+' · Notification '+notification.status);audit_(session,'ADMIN_PERMANENT_BADGE_STATUS',found.obj.VisitID,found.obj.ConfirmationNumber,auditStatus,details+' · Notification '+notification.status);return{ok:true,message:'Permanent badge request status updated. Stakeholder notification: '+notification.status+'.',notification:notification,request:publicPermanentBadgeRequest_(updated)};}
function notifyPermanentBadgeStatus_(record){const recipients=[record.SponsorEmail,record.VisitorEmail].filter(validEmailAddress_).filter((v,i,a)=>a.indexOf(v)===i);if(!recipients.length)return{sent:false,status:'No recipients'};try{const body=`<p>The permanent badge request for <strong>${html_(record.VisitorName)}</strong> has moved to <strong>${html_(record.Status)}</strong>.</p>${permanentBadgeRequestDetails_(record)}${record.ProcessingNotes?`<p style="padding:13px 15px;background:#f3f8fd;border-radius:8px"><strong>Global Security update</strong><br>${html_(record.ProcessingNotes)}</p>`:''}`;MailApp.sendEmail({to:recipients.join(','),subject:`Permanent badge ${record.RequestID}: ${record.Status}`,body:`Permanent badge request ${record.RequestID} is now ${record.Status}. ${record.ProcessingNotes||''}`,htmlBody:brandedEmail_({title:'Permanent Badge Status Update',kicker:'FORD ENERGY · VISTA GLOBAL SECURITY',body:body})});return{sent:true,status:'Sent',recipients:recipients};}catch(e){return{sent:false,status:'Failed',error:String(e&&e.message||e)};}}
function createSponsoredVisitAuthorized_(p,session){
  ...
  try{
    for(let week=0;week<repeatWeeks;week++){
      ...
      Object.assign(record,advanceNoticeMetrics_(now,dateOnly_(start),timeOnly_(start)));
...
function reminderRecipientsForVisit_(visit,type,now){const cfg=config_(),base=[String(visit.SponsorEmail||'').trim().toLowerCase()].concat(delegateEmailsForVisit_(visit,now)),leaders=activeSponsorLeadership_().filter(r=>truthyActive_(r.NotificationsEnabled)&&sponsorIdentityMatchesVisit_(visit,{Email:r.SponsorEmail,EmployeeID:r.SponsorEmployeeID,Username:r.SponsorUsername})).map(r=>String(r.LeaderEmail||'').trim().toLowerCase()),escalation=type==='Escalation'?facilitiesRecipients_(cfg.SPONSOR_ESCALATION_EMAILS).emails.concat(leaders):[],all=[...new Set(base.concat(escalation).filter(validEmailAddress_))];return{all:all,sponsor:String(visit.SponsorEmail||'').trim().toLowerCase(),delegates:[...new Set(delegateEmailsForVisit_(visit,now))],escalation:[...new Set(escalation)]};}
...
function updateVisitStatusAuthorized_(p,session){
  ...
  if(status==='Approved'){
    requirePermission_(session,'approve');
    validateRequired_(p,['notes','parkingLotAssignment','officeSeatingAssignment']);
  ...
  if(status==='Approved'){
    const decisionAt=new Date(),created=parseDateSafe_(rec.CreatedAt),role=String(session.Role||''),source=sponsorRole_(session)?'Sponsor / Approver Portal':(['Admin','Super Administrator'].includes(role)?'Administration Oversight':'Security Operations'),approvalUpdates={SponsorApprovalNotes:note,ParkingLotAssignment:String(p.parkingLotAssignment).trim(),OfficeSeatingAssignment:String(p.officeSeatingAssignment).trim(),ApprovalDecisionAt:decisionAt,ApprovalResponseMinutes:created?Math.max(0,Math.round((decisionAt-created)/60000)):0,ApprovalDecisionBy:String(session.FullName||session.Username||''),ApprovalDecisionRole:role,ApprovalSource:source,ApprovalManualOverride:sponsorRole_(session)?'No':'Yes',LastUpdatedAt:decisionAt};
    updateVisitRow_(findVisitRow_(p.visitId).row,approvalUpdates);Object.assign(rec,approvalUpdates);
  }
...
function updateSponsoredVisitAuthorized_(p,session){
  requirePermission_(session,'approve');validateRequired_(p,['visitId']);const row=requireSponsorOwnedVisit_(p.visitId,session),visit=row.obj,current=String(visit.Status||'Submitted');
  ...
  Object.assign(updates,advanceNoticeMetrics_(visit.CreatedAt,startDate,arrival));updates.LastUpdatedAt=new Date();updateVisitRow_(row.row,updates);const details=['Sponsor updated visit details',String(p.notes||'').trim()].filter(Boolean).join(' · ');logActivity_(p.visitId,'SPONSOR_VISIT_UPDATED',session.FullName||session.Username,'',details);auditVisit_(session,'SPONSOR_VISIT_UPDATED',p.visitId,'Success',details);return{ok:true,visit:publicVisit_(Object.assign({},visit,updates))};
}
...
function normalizeVistaRole_(role){
  ...
  const roles={superadministrator:'Super Administrator',superadmin:'Super Administrator',admin:'Admin',securitysupervisor:'Security Supervisor',security:'Security',securityofficer:'Security',frontdesk:'FrontDesk',sponsor:'Sponsor',approver:'Approver',plantleader:'Plant Leader',ll5:'Plant Leader',reporting:'Reporting',readonly:'Read Only'};
...
function formatVisitGovernanceColumns_(sheet){if(!sheet||sheet.getMaxRows()<2)return;const headers=sheet.getRange(1,1,1,sheet.getLastColumn()).getValues()[0],formats={AdvanceNoticeRequiredDays:'0',AdvanceNoticeActualDays:'0.0',AdvanceNoticeHours:'0.0',ApprovalResponseMinutes:'0'};Object.keys(formats).forEach(header=>{const index=headers.indexOf(header);if(index>=0)sheet.getRange(2,index+1,sheet.getMaxRows()-1,1).setNumberFormat(formats[header]);});}
