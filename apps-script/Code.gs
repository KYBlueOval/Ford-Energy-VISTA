const FE = {
  VERSION: '2.5.5-sponsor-performance',
...
function setupVisitorManagement() {
  ...
  return 'VISTA 2.5.5 setup complete. Advance-notice governance, mandatory approval logistics, sponsor performance analytics, and leadership outlook reporting are ready.';
}
...
function sponsorPerformance_(visits){
  const groups={};
  visits.forEach(r=>{const email=String(r.SponsorEmail||'').trim().toLowerCase(),name=String(r.SponsorName||'Unassigned').trim()||'Unassigned',key=email||name.toLowerCase(),notice=advanceNoticeForVisit_(r),status=String(r.Status||'Submitted'),decided=['Approved','Checked In','Checked Out','Denied','Rejected','Cancelled','No Show'].includes(status),approved=['Approved','Checked In','Checked Out'].includes(status),response=Number(r.ApprovalResponseMinutes||0);if(!groups[key])groups[key]={name:name,email:email,totalRequests:0,compliantRequests:0,lateRequests:0,pendingRequests:0,approvedRequests:0,deniedRequests:0,manualApprovals:0,responseTotal:0,responseSamples:0,leadTotal:0};const g=groups[key];g.totalRequests++;notice.compliant?g.compliantRequests++:g.lateRequests++;g.leadTotal+=Number(notice.actualDays||0);if(!decided)g.pendingRequests++;if(approved)g.approvedRequests++;if(['Denied','Rejected'].includes(status))g.deniedRequests++;if(approved&&String(r.ApprovalManualOverride||'')==='Yes')g.manualApprovals++;if(response>0){g.responseTotal+=response;g.responseSamples++;}});
...
function advanceNoticeMetrics_(createdAt,startDate,arrivalTime){const required=advanceNoticeRequiredDays_(),created=parseDateSafe_(createdAt)||new Date(),scheduled=scheduledVisitDate_(startDate,arrivalTime),hours=scheduled?Math.round((scheduled-created)/360000)/10:0,days=Math.round(hours/24*10)/10,compliant=Boolean(scheduled&&hours>=required*24);return{AdvanceNoticeRequiredDays:required,AdvanceNoticeActualDays:days,AdvanceNoticeHours:hours,AdvanceNoticeCompliant:compliant?'Yes':'No',AdvanceNoticeFlaggedAt:compliant?'':created};}
function advanceNoticeForVisit_(r){const required=String(r.AdvanceNoticeRequiredDays||'')!==''?Number(r.AdvanceNoticeRequiredDays):advanceNoticeRequiredDays_(),created=parseDateSafe_(r.CreatedAt),scheduled=scheduledVisitDate_(r.StartDate,r.ArrivalTime),hours=String(r.AdvanceNoticeHours||'')!==''?Number(r.AdvanceNoticeHours):(created&&scheduled?Math.round((scheduled-created)/360000)/10:0),days=String(r.AdvanceNoticeActualDays||'')!==''?Number(r.AdvanceNoticeActualDays):Math.round(hours/24*10)/10,compliant=String(r.AdvanceNoticeCompliant||'')?String(r.AdvanceNoticeCompliant)==='Yes':hours>=required*24;return{requiredDays:required,actualDays:days,hours:hours,compliant:compliant};}
