(()=>{
  ...
  function renderShiftHandoffs(rows){const host=$('#handoffRows');host.innerHTML=rows.map(h=>`<article class="handoff-row"><div class="handoff-meta"><strong>${esc(h.shiftLabel||'Shift handoff')}</strong><span>${esc(h.createdAt)} · ${esc(h.createdBy||'Unknown officer')}</span><em class="handoff-status">${esc(h.status)}</em></div><div class="handoff-note">${esc(h.notes)}</div><div class="handoff-counts"><span>${h.visitorsOnsite} onsite</span><span>${h.badgesOut} badges out</span><span class="${h.overdueVisitors?'attention':''}">${h.overdueVisitors} overdue</span><span class="${h.badgeExceptions?'attention':''}">${h.badgeExceptions} exceptions</span></div></article>`).join('');$('#handoffEmpty').classList.toggle('hidden',rows.length>0)}
