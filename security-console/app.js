(()=>{
  ...
  async function load(){
    ...
    try{
      ...
      const d=await api('listVisits',{query:q,status});records=d.visits||[];renderRows();renderKpis(d.kpis||{});await Promise.all([loadActiveOperations(),loadShiftHandoffs()]);
  ...
  async function loadShiftHandoffs(){
    try{const d=await api('listShiftHandoffs',{limit:5});renderShiftHandoffs(d.handoffs||[])}catch(e){$('#handoffRows').innerHTML=`<p class="active-empty">Handoff history unavailable: ${esc(e.message)}</p>`;$('#handoffEmpty').classList.add('hidden')}
  }
  ...
  $('#activeRefresh').onclick=loadActiveOperations;$('#handoffRefresh').onclick=loadShiftHandoffs;$('#submitHandoff').onclick=createShiftHandoff;setInterval(updateActiveTimers,30000);
